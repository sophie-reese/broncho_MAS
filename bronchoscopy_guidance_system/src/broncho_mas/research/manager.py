"""
Research manager adapted from the older legacy MAS manager.

Design goal:
- Keep the original working flow as intact as possible.
- Only make small compatibility changes so it fits the newer package layout.
- Add light robustness for tool-wrapper outputs and curriculum fallback.

Main changes:
1. Import paths adapted for new layout (shared/ vs research/).
2. Tool-wrapper outputs like {"tool": "...", "arguments": {...}} are unwrapped.
3. If shared CurriculumEngine is incomplete, fall back to research.curriculum_engine.
4. Slightly stricter plain-text fallback for online.py compatibility.
"""
...
from .logging_utils import MASLogger
...
from .timeline_io import load_session_metrics
from .reporting import build_core_report, report_has_required_structure
...
try:
    ...
    from .prompts import build_report_prompt, build_statistics_prompt, build_instructor_prompt  # type: ignore


def _extract_json_object(text: Union[str, Dict, Any]) -> Dict[str, Any]:
...
def _unwrap_tool_arguments(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if obj.get("name") == "submit_guidance" and isinstance(obj.get("arguments"), dict):
        return obj["arguments"]

    fn = obj.get("function")
    if isinstance(fn, dict) and fn.get("name") == "submit_guidance":
...
def _coerce_instructor_result(raw: Any) -> Dict[str, Any]:
    """Best-effort recovery of instructor business payload."""
...
class MultiAgentManager:
    """Bronchoscopy MAS manager (ordered curriculum, legacy-safe output, new-layout compatible)."""
    ...
    def __init__(self, model_name: str | None = None):
        ...
        self._turn_idx = 0
        self._last_ui_text = ""
        self.mas_logger = MASLogger()
        try:
            self.mas_logger.write_meta(
                {
                    "broncho_mas_version": "0.0.3-research",
        ...
        print(
            f"[broncho_mas manager] version=0.0.3-research "
    ...
    def _make_engine(self):
        # Prefer shared curriculum only if it is actually complete enough.
    ...
    def build_statepacket(
        ...
        return {
            ...
            "signals": {"eeg": None},
            "meta": {"broncho_mas_version": "0.0.3-research"},
        }
    ...
    @staticmethod
    def _extract_visual_context(text: str) -> Dict[str, Any]:
        s = str(text or "")
        def _grab(label: str) -> str:
            m = re.search(rf"{label}\s*:\s*([A-Za-z0-9+]+)", s, flags=re.I)
    ...
    def _content_conflicts(self, text: str, visual_ctx: Dict[str, Any]) -> bool:
        ...
        if not s:
            return True
        target_family = self._airway_family(visual_ctx.get("target_region", ""))
    ...
    def _bedside_safe_guidance(
        ...
        current_region = str((visual_ctx or {}).get("current_region", "")).upper()
        target_region = str((visual_ctx or {}).get("target_region", "")).upper()
    ...
    def run(self, prompt: str) -> Dict[str, Any]:
        ...
        cp = _ensure_dict(plan.get("curriculum_progress", {}))
        cp = _unwrap_tool_arguments(cp)
        lh = _ensure_dict(plan.get("landmark_hint", {}))
        lh = _unwrap_tool_arguments(lh)

        deterministic_next = self.engine.next_airway({str(x).upper() for x in reached_list})
        if deterministic_next:
            cp["next_airway"] = deterministic_next
        elif "next_airway" not in cp:
            cp["next_airway"] = ""
        ...
        # tolerate alternate field names from model/tool wrappers
        if stats_obj:
        ...
        has_visual_context = bool(visual_ctx.get("target_region") or visual_ctx.get("current_region"))
            ...
            self.mas_logger.append_turn(
                {
                    ...
                    "outputs": {
                        "statistics": {"raw": stats_raw, "validated": stats_valid, "errors": stats_errs},
                        "instructor": {
                            "raw": instr_raw,
        ...
        return {
            ...
            "statepacket": statepacket,
            "raw": {"stats_raw": stats_raw, "instr_raw": instr_raw},
        }
    ...
    @staticmethod
    def _extract_reached_regions(current_situation: str) -> List[str]:
        import ast
        import re
