...
# Prefer explicit field-style inputs, but keep legacy phrasing support.
_CURRENT_PATTERNS = [
...
class RealtimeInstructorEngine:
    ...
    def __init__(self, curriculum_engine: CurriculumEngine):
        ...
        self._allowed_airways = set(getattr(curriculum_engine, "airway_order", []) or getattr(curriculum_engine, "AIRWAY_VISIT_ORDER", []) or [])
    ...
    def build_plan(self, ctx: RealtimeContext) -> Dict[str, Any]:
        ...
        if ctx.needs_backtrack:
            ...
            cue = getattr(landmark, "recognition_cue", "") or "the next opening"
            first_angle = "0°"
            ...
            cue = getattr(landmark, "recognition_cue", "") or "the next lumen"
            micro_steps = [
                "Keep the lumen centered.",
    ...
    def build_guidance(self, ctx: RealtimeContext, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cue = str(plan.get("recognition_cue", "")).strip()
