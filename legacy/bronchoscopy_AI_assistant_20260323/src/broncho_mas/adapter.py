...
def _strict_pipeline_mode() -> bool:
    return str(os.environ.get("BRONCHO_STRICT_PIPELINE", "")).strip().lower() in {"1", "true", "yes", "on"}
...
def _build_manager(model_name: str):
    ...
    try:
        ...
        _warn_fallback(f"unknown pipeline '{pipeline}' could not resolve to runtime, falling back to MAS", exc)
        if _strict_pipeline_mode():
            raise
        from .mas.manager import MASManager  # type: ignore
        return MASManager(model_name=model_name)
...
class SmolAgentsLLM:
    ...
    def get_report(self, recording_dir: Optional[str] = None) -> str:
        ...
        raise NotImplementedError(
            "The active pipeline does not implement get_report(). "
            "Set BRONCHO_PIPELINE=mas for report generation."
        )
